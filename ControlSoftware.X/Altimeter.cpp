...
bool HAL::Altimeter::Initialize() {
    ...
    //If we got actual data back. Note that there is no safeguard against junk.
        ...
        PressureRaw |= ((UnsignedInteger32)PressureArray[2] << 20);
...
bool HAL::Altimeter::Update() {
    ...
    GetTemperatureCommands[0] = (PRES_ADDRESS + 0x0200);
    GetTemperatureCommands[1] = (PRES_ADDRESS + 0x0100);
    GetTemperatureCommands[2] = PRES_ADDRESS;
    ...
    if (TempArray == 0) {
        ...
        TemperatureRaw |= ((SignedInteger32)TempArray[2] << 20);
